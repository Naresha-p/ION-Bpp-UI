...
// ─── Item → form helper (used by EditModal) ───────────────────────────────────
...
function EditModal({ item, onClose, onSaved }) {
  ...
  const handleSubmit = async () => {
    ...
    if (Object.keys(errs).length) {
      setErrors(errs)
      const coreFields = ['providerId','name','shortDesc','unitPrice','brand','originCountry','weightQty','cuisine','prepInstructions']
    ...
    <div className="modal-backdrop">
      <div className="modal-panel w-full max-w-2xl max-h-[92vh] flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                    ...
                    {ALLERGENS.map((a) => (
                      <button key={a} type="button" onClick={() => toggleAllergen(a)}
                        className={`text-xs px-2.5 py-1 rounded-full border font-medium transition-colors ${
                          form.allergens.includes(a) ? 'tag-pill active-red' : 'tag-pill'
                        }`}>
                        {a}
                      </button>
                    ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div>
                    ...
                    <input className={`beckn-input ${errors.prepInstructions ? 'border-red-400' : ''}`}
                      value={form.prepInstructions} onChange={(e) => set('prepInstructions', e.target.value)} />
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--text-2)' }}>
                    <input type="checkbox" checked={form.isActive}
                      onChange={(e) => set('isActive', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                    Active
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--text-2)' }}>
                    <input type="checkbox" checked={form.isPublished}
                      onChange={(e) => set('isPublished', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                    Published
                  </label>
                </div>
              </div>
          ...
          {/* ── Policies & Payment ── */}
          <div>
            <SectionHeader title="Policies & Payment" open={sections.policies} onToggle={() => toggleSection('policies')} />
            {sections.policies && (
              <div className="space-y-4">
                <div className="rounded-xl p-3 space-y-2" style={{ border: '1px solid var(--border)', background: 'var(--surface-r)' }}>
                  <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold" style={{ color: 'var(--text-2)' }}>
                    <input type="checkbox" checked={form.returnsAllowed}
                      onChange={(e) => set('returnsAllowed', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                    Returns Allowed
                  </label>
                  {form.returnsAllowed && (
                ...
                <div className="rounded-xl p-3 space-y-2" style={{ border: '1px solid var(--border)', background: 'var(--surface-r)' }}>
                  <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold" style={{ color: 'var(--text-2)' }}>
                    <input type="checkbox" checked={form.cancellationAllowed}
                      onChange={(e) => set('cancellationAllowed', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                ...
                <div className="rounded-xl p-3 space-y-2" style={{ border: '1px solid var(--border)', background: 'var(--surface-r)' }}>
                  <label className="flex items-center gap-2 cursor-pointer text-sm font-semibold" style={{ color: 'var(--text-2)' }}>
                    <input type="checkbox" checked={form.replacementAllowed}
                      onChange={(e) => set('replacementAllowed', e.target.checked)} className="w-4 h-4 accent-blue-600" />
                    Replacement Allowed
                  </label>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {PAYMENT_METHODS.map((m) => (
                      <button key={m} type="button" onClick={() => togglePayment(m)}
                        className={`text-xs px-2.5 py-1 rounded-full border font-medium transition-colors ${
                          form.paymentMethods.includes(m) ? 'tag-pill active' : 'tag-pill'
                        }`}>
                        {m}
                      </button>
                    ))}
                ...
                <label className="flex items-center gap-2 cursor-pointer text-sm" style={{ color: 'var(--text-2)' }}>
                  <input type="checkbox" checked={form.codAvailable}
                    onChange={(e) => set('codAvailable', e.target.checked)} className="w-4 h-4 accent-blue-600" />
          ...
          {/* ── Serviceability ── */}
          <div>
            <SectionHeader title="Serviceability" open={sections.serviceability} onToggle={() => toggleSection('serviceability')} />
                  ...
                  <div className="flex gap-1.5 mt-1 flex-wrap">
                    {DAYS.map((d) => (
                      <button key={d} type="button" onClick={() => toggleDay(d)}
                        className={`text-xs px-2.5 py-1 rounded-full border font-medium transition-colors ${
                          form.timingDays.includes(d) ? 'tag-pill active-dark' : 'tag-pill'
                        }`}>
                        {d}
                      </button>
                    ))}
...
function ProductCard({ item, isSelected, isPublished, onToggle, onEdit }) {
    ...
    <div
      ...
      <div className="p-3 space-y-1.5">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg text-xs font-semibold mt-1 transition-all"
...
export default function PublishPage() {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {editItem && (
        <EditModal item={editItem} onClose={() => setEditItem(null)} onSaved={handleSaveEdit} />
      )}
      {showConfirm && (
        <ConfirmPublishModal
          items={selectedList}
